...
public class LaserRenderer extends SmartBlockEntityRenderer<LaserBlockEntity> {
    ...
    protected void renderSafe(LaserBlockEntity be, float partialTicks, PoseStack ps, MultiBufferSource buffer, int light, int overlay) {
        ...
        *///?} else {
        ...
        vc.addVertex(pose, (float) v1.x, (float) v1.y, (float) v1.z).setColor(1f, 1f, 1f, 1f)
                ...
                .setNormal(pose, nx, ny, nz);
        vc.addVertex(pose, (float) v2.x, (float) v2.y, (float) v2.z).setColor(1f, 1f, 1f, 1f)
    ...
    *///?} else
    ...
    private static void renderPart(PoseStack ps, VertexConsumer c,
        ...
        PoseStack.Pose pose = ps.last();
        renderBeamQuad(pose, c, r, g, b, a, minY, maxY, x0, z0, x1, z1, minU, maxU, minV, maxV);
        renderBeamQuad(pose, c, r, g, b, a, minY, maxY, x3, z3, x2, z2, minU, maxU, minV, maxV);
        renderBeamQuad(pose, c, r, g, b, a, minY, maxY, x1, z1, x3, z3, minU, maxU, minV, maxV);
        renderBeamQuad(pose, c, r, g, b, a, minY, maxY, x2, z2, x0, z0, minU, maxU, minV, maxV);
    }

    private static void renderBeamQuad(PoseStack.Pose pose, VertexConsumer c,
                                        float r, float g, float b, float a,
                                        int minY, int maxY,
                                        float minX, float minZ, float maxX, float maxZ,
                                        float minU, float maxU, float minV, float maxV) {
        beamVertex(pose, c, r, g, b, a, maxY, minX, minZ, maxU, minV);
        beamVertex(pose, c, r, g, b, a, minY, minX, minZ, maxU, maxV);
        beamVertex(pose, c, r, g, b, a, minY, maxX, maxZ, minU, maxV);
        beamVertex(pose, c, r, g, b, a, maxY, maxX, maxZ, minU, minV);
    }

    private static void beamVertex(PoseStack.Pose pose, VertexConsumer c,
                                    float r, float g, float b, float a,
